...
function createContainer(w, h, el, altText, opts) {
  var alt;
  var container = d.createElement('BUTTON');
  ...
  cls ? container.setAttribute('class', el.getAttribute('class')) : null;
  id ? container.setAttribute('id', el.getAttribute('id')) : null;
...
function process(el, gifs, options) {
  ...
  isCanvas = !!(c.getContext && c.getContext('2d'));
  if (w && h && isCanvas) cc = createContainer(w, h, el, altText, options);
  ...
  el.onload = function() {
    ...
    canvas.getContext('2d').drawImage(el, 0, 0, dims.w, dims.h);
